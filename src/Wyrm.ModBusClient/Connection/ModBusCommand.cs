    ...
    IModBusConnection _modBusConnection) : IModBusCommand
{
    ...
    public async ValueTask<ICollection<bool>> ReadBitValuesAsync(byte functionNumber, ushort startingAddress, ushort bitsToRead, CancellationToken cancellationToken)
    {
        ...
        return GetBitValues(bitsData, bitsToRead);
    }
    ...
    public async ValueTask<DeviceIdentifierResult> ReadDeviceIdentifiersAsync(byte functionNumber, byte readDeviceId, byte objectId, CancellationToken cancellationToken)
    {
        ...
        return MakeDeviceIdentifierResult(identifierSpan);
    }
    ...
    public ValueTask<ushort> ReadUshortValuesRequestAsync(byte functionNumber, ushort startingAddress, ushort ushortsToRead, CancellationToken cancellationToken)
    {
        if (ushortsToRead > MaximumUshortValues)
            throw new ModBusClientException("ModBus Client: Too many values requested.", ModBusExceptionCode.TooManyUshortValues);

        return _modBusConnection.RequestFunctionAsync(functionNumber, [startingAddress, ushortsToRead], [], cancellationToken);
    }

    public async ValueTask<UshortDataResponse> ReadUshortValuesResponseAsync(CancellationToken cancellationToken)
    {
        var functionData = await _modBusConnection.PerformReadAsync(cancellationToken);

        return new UshortDataResponse
        {
            ...
            UshortData = GetUshortValues(functionData.Data, (ushort)(functionData.Data.Span[0] / 2))
        };
    }
    ...
    private static ICollection<bool> GetBitValues(ReadOnlyMemory<byte> bitsData, ushort bitsToRead)
    {
    ...
    private static ICollection<ushort> GetUshortValues(ReadOnlyMemory<byte> ushortsData, ushort ushortsToRead)
    {
        var ushortValues = new List<ushort>();
    ...
    private static DeviceIdentifierResult MakeDeviceIdentifierResult(ReadOnlySpan<byte> identifierSpan)
    {
